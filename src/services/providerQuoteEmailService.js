...
export const buildProviderQuoteEmailDraft = ({
  ...
  const providerEmails =
    uniqueEmails(providers);

  console.log(
    "[FixBee][QuoteEmail] building provider quote preview",
    {
      providerCount: providers.length,
      providerEmailCount:
        providerEmails.length,

      hasUploadedImageUrl: Boolean(
  ...
  const suppliedImages =
    Array.isArray(images)
      ? images
          .map(normalizeQuoteImage)
          .filter(Boolean)
      : [];

  const shouldUseSuppliedImages =
    preferImages ||
  ...
  const plainBody =
    buildPlainQuoteEmailBody({
      issueTitle,
      detectedObject:
        clean(detectedObject),

      urgency:
        clean(urgency),

      estimatedCostRange:
        clean(estimatedCostRange),

      estimatedRepairTime:
        clean(estimatedRepairTime),
  ...
  return {
    ...
    cc:
      clean(requesterEmail) ||
      "Logged-in user email will be added by backend",
...
const mapProviderForRequest = (
  ...
  return {
    ...
    phoneDisplay:
      clean(provider.phoneDisplay),

    city:
      clean(provider.city),

    address:
      clean(provider.address),
  };
};
...
export const sendProviderQuoteRequestFromPreview =
  async (payload) => {
    console.log(
      "[FixBee][QuoteRequest] sending official backend quote request",
      {
        providerCount:
          payload?.providers
            ?.length || 0,
    ...
    const result =
      await sendProviderQuoteRequest(
        payload
      );

    console.log(
      "[FixBee][QuoteEmail] official backend quote request sent",
      {
        quoteRequestId:
